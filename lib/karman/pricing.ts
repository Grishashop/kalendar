...
export function computePrice(input: PriceInput): PriceResult {
  ...
  // Рыночная заявка на FORTS подаётся с наихудшей допустимой ценой — планкой дня.
  const worst = isBuy ? highLimit : lowLimit;
  if (worst === null) {
    const missing = "неизвестна предельная цена рыночной заявки";
    return { type: "market", price: null, reason: reason === null ? missing : `${reason}; ${missing}` };
  }

  return { type: "market", price: clamp(worst), reason };
}

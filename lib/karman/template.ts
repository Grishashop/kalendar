...
export interface TemplateRoles {
  ...
  /**
   * Значения колонки стороны во вставке заявок, означающие покупку, через
   * запятую. Это словарь QUIK, и он ДРУГОЙ, чем наш: в таблице заявок стоит
   * «Купля», а при включённом формальном представлении — код `B`. Склеивать
   * с `directionBuyLabel` нельзя: покупки посчитались бы продажами.
   */
  orderBuyLabel: string;
  /** Заявки: колонка состояния. Снимать имеет смысл только живые заявки. */
  orderStatus: string;
  /**
   * Значения состояния, означающие живую заявку, через запятую. Как и сторона,
   * записывается по-разному в зависимости от настроек буфера: `ACTIVE` или
   * «Активна». Незнакомое значение блокируется регуляркой колонки — принять
   * его за исполненное значит не снять живую заявку и уехать на поставку.
   */
  orderActiveStatus: string;
  /** Стоп-заявки: привязка к позиции. Сверить объём не с чем. */
  stopAccount: string;
  stopInstrument: string;
  /** Стоп-заявки: колонка состояния и живые значения — как у обычных заявок. */
  stopStatus: string;
  stopActiveStatus: string;
}
...
const FORTS_LINE = [
  ...
  // UID вводится на странице, а не правится здесь: файлом пользуются несколько
  // трейдеров. Пустой UID пустит поле целиком — маркер «/&!» без значения
  // отправлять незачем.
  'Комментарий=${IF([UID]="";"";CONCATENATE("/&!";[UID]))}',
...
export const BUILTIN_TEMPLATES: readonly Template[] = [
  {
    ...
    columns: [
      ...
      // которые типовая проверка сама по себе не различает. Тикер до двенадцати
    ...
    orderColumns: [
      ...
      // Та же настройка меняет и запись стороны, и запись состояния: при
      // формальном представлении это коды, при обычном — русские слова.
      // Регулярка ловит незнакомое значение, иначе оно молча посчиталось бы
      // продажей и сломало сверку объёма.
      { name: "Операция", type: "text", pattern: "^(Купля|Продажа|B|S)$" },
      // Остаток, а не исходный объём: частично исполненную заявку связывает
      // именно он, и он же стоит в «Акт. покупка» и «Акт. продажа».
      { name: "Остаток", type: "number" },
      // Незнакомое состояние блокирует генерацию: принять его за исполненное
      // значит не снять живую заявку и уехать на поставку.
      { name: "Состояние", type: "text", pattern: "^(ACTIVE|FILLED|KILLED|Активна|Исполнена|Снята)$" },
    ],
    stopColumns: [
      { name: "Торговый счет", type: "text", pattern: "^[A-Za-z0-9_-]{6,16}$" },
      { name: "Код инструмента", type: "text", pattern: "^[A-Za-z0-9._-]{2,16}$" },
      ...
      // Снятая или исполненная стоп-заявка ничего не блокирует; снимать её —
      // лишняя отвергнутая транзакция в кармане.
      { name: "Состояние", type: "text", pattern: "^(ACTIVE|FILLED|KILLED|Активна|Исполнена|Снята)$" },
    ],
    values: {
    ...
    roles: {
      ...
      orderSide: "Операция",
      orderQuantity: "Остаток",
      // В таблице заявок QUIK стоит «Купля», а при формальном представлении
      // данных — код «B». Оба написания проверены на выгрузке терминала.
      orderBuyLabel: "Купля, B",
      orderStatus: "Состояние",
      // Снимаем только то, что ещё живо. Оба написания проверены на выгрузке.
      orderActiveStatus: "ACTIVE, Активна",
      stopAccount: "Торговый счет",
      stopInstrument: "Код инструмента",
      stopStatus: "Состояние",
      stopActiveStatus: "ACTIVE, Активна",
    },

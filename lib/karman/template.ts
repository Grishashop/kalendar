...
export const BUILTIN_TEMPLATES: readonly Template[] = [
  {
    ...
    orderColumns: [
      ...
      // Та же настройка меняет и запись стороны: при формальном представлении
      // это коды B/S, при обычном — слова. Регулярка ловит незнакомое значение,
      // иначе оно молча посчиталось бы продажей и сломало сверку объёма.

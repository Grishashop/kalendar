...
/**
 * Снимок живёт 30 секунд. Замер показал, что запрос десяти строк и запрос одной
 ...
 *
 * Тридцать секунд, а не пять: у проекта Supabase заканчивается квота, и цена
 * свежести — трафик. Обновление ленивое, по приходу запроса, а не по таймеру,
 * поэтому потолок — два обращения в минуту на инстанс вместо двенадцати.
 * Плата названа прямо: отозванный доступ доживает до тридцати секунд.
 */
